...
class DeepSeekProvider(BaseProvider):
    ...
    async def generate(self, request: ProviderRequest) -> ProviderResponse:
        return ProviderResponse(text="DeepSeek response", model_id=request.model_id, latency_ms=0, usage=TokenUsage(input_tokens=0, output_tokens=0), finish_reason=FinishReason.STOP)
